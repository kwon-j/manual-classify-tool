...
input_folder = str((scriptdir / 'train' / 'n03240683').resolve()) + "/"

# the different folders into which you want to sort the images, e.g. ['cars', 'bikes', 'cats', 'horses', 'shoes']
labels = ["water", "no_water", "idk"]  # don't change idk class to NA, as this will be read in pandas as NaN/Null

# A file-path to a csv-file, that WILL be created by the script.
# The results of the manual classification will be stored there.
...
df_path = input_folder + "labels.csv"
...
class ImageGui:
    ...
    def __init__(self, master, labels, paths, df_path):
        """
        Initialise GUI
        ...
        ####
        try:
            self.df = pd.read_csv(df_path, index_col=0, header=0)
            # Store configuration file values
        except FileNotFoundError:
            self.df = pd.DataFrame(columns=["im_path", 'label'])
        ...
        #### added in version 2
        # Add sorting label
        sorting_string = os.path.split(self.df.im_path[self.index])[-2]
    ...
    def show_next_image(self):
        ...
        if self.index < self.n_paths:
            ...
            sorting_string = os.path.split(self.df.im_path[self.index])[-2] #shows the last folder in the filepath before the file
    ...
    def move_prev_image(self):
        ...
        if self.index < self.n_paths:
            self.set_image(self.df.im_path[self.index]) # change path to be out of df
        else:
            self.master.quit()
    ...
    def vote(self, label):
        ...
        # if (self.df["label"].isnull().values.any()) | ((self.df["label"] == '').values.any()):
        ...
        self._sort_image(label, self.index)
    ...
    def calc_index(self):
        idx = self.df[(self.df["label"] == '') | (self.df["label"].isnull())].index.tolist()
        return(idx[0])
    ...
    def _sort_image(self, label, ind):
        """
        ...
        """
        _, file_name = os.path.split(self.df.im_path[ind])
    ...
    #     print("file_name =",file_name)
    #     print(" %s --> %s" % (file_name, label))
    #     move(df.im_path[ind], output_path)
    #
    #     # keep track that the image location has been changed by putting the new location-path in sorted_in_folder
...
#     labels = args.labels
    ...
    # Start the GUI
    root = tk.Tk()
    app = ImageGui(root, labels, paths, df_path)
    root.mainloop()
